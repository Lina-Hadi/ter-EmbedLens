...
def compute_metrics(embeddings, coords_2d, labels, k=5):
    ...
    # 3) Trustworthiness : respect du voisinage local entre 512D et 2D
    trust_score = trustworthiness(embeddings, coords_2d, n_neighbors=k)

    # 4) k-NN Accuracy sur les embeddings originaux (512D)
...
def main():
    ...
    np.save("./data/processed/embeddings_tsne.npy", embeddings)
    np.save("./data/processed/labels_tsne.npy", labels)
    ...
    np.save("./data/processed/coords_2d_tsne.npy", coords_2d)
    ...
    with open("./data/processed/tsne_metrics.txt", "w", encoding="utf-8") as f:
        f.write("===== METRIQUES =====\n")
    ...
    # 6) Visualisation avec métriques affichées sur la figure
    ...
    plt.title("CIFAR-10 - t-SNE 2D (CLIP embeddings)")
    plt.xlabel("t-SNE 1")
    plt.ylabel("t-SNE 2")

    # Laisser de l'espace à droite pour les métriques
    ...
    plt.savefig("./data/processed/tsne_plot_with_metrics.png", dpi=150)
    plt.show()

    print("Plot sauvegardé dans ./data/processed/tsne_plot_with_metrics.png")
    print("Métriques sauvegardées dans ./data/processed/tsne_metrics.txt")
